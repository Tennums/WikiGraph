...
for the host's own buttons, and a node's `mark` ("toread" | "read") is drawn as a halo, and an edge with weight 2 (the host's "new since last month") in gold. `page.css` gained the `.preview`
